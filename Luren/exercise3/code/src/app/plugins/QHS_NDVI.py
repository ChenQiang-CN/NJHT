# -*- coding:utf-8 -*-
# Created on 2021/8/17 by Lu Ren
...
from src.common.utils.GdalUtils.CloudDection import CloudDection
...
from src.app.plugins.rsdata_process import tiff_resample, cal_ndvi


class QHS_NDVI(BaseProcess):
    """
    青海省HDVI产品
    input: 葵花8.tif
    ...
    def __init__(self, pluginParam):
        ...
        self.excel_title = [["地区", "总和", "最大值", "最小值", "平均值"]]
        self.table_title = "{0}NDVI".format(self.issue[:4])
        self.proexcelcols = [["name", "SUM", "MAX", "MIN", "MEAN"]]
        self.prowordcols = [["name", "SUM", "MAX", "MIN", "MEAN"]]
    ...
    def doInnerPy(self):
        ...
        output_path = "F:/HTHT/NJHT/Luren/exercise3/data/Output/"
        output_file = os.path.join(output_path, "VGT_NDVI_RCUR_202007220400_COOD_H8_AHI_QHS.tif")
        # 重采样（预处理后的数据不需要）
        # tiff_resample(input_file, 0.01, output_path + 'resample_temp.tif')
        # resample_file = os.path.join(output_path, 'resample_temp.tif')
        im_data, im_width, im_height, im_bands, im_geotrans, im_proj = GdalBase.read_tiff(input_file)
        red_band = im_data[2]  # 0.64, reso = 0.5 km
        nir_band = im_data[3]  # 0.86, reso = 1 km
        # t12_band = im_data[14]  # 0.86, reso = 1 km
        ndvi = cal_ndvi(nir_band, red_band, im_data[4], im_data[2])
        # 云掩膜模块
        # cloud_mask = CloudDection.Cloud_Dection_Physical(red_band,nir_band,t12_band)
        # ndvi[cloud_mask==1] = -9999
        temp_tif = os.path.join(self.tempDir, "temp.tif")
        GdalBase.write_tiff(ndvi, im_width, im_height, 1, im_geotrans, im_proj, temp_tif)
        # 转换投影
        tifPath_alber = self.tifProjChange(temp_tif)
        GdalUtil.rasterClipByShp(tifPath_alber, output_file, shpPath)  # 裁切青海省范围
        self.rsOutMap["OUTFILEPATH"] = output_file
    ...
    def pdToExcel(self, mean_sub_df, area_id):
        ...
        excelPath = os.path.join(self.curProdDir, '{id}', basename + '.xls')
    ...
    def exportWord(self, areaList):
        """生成文档"""
        docDir = os.path.join(self.pluginParam.getDependDir(), 'word/QHS_NDVI/')
...
if __name__ == "__main__":
    # 该模块实现了青海省NDVI的计算功能
    # 模型输入数据：H08_20210722_0400_MULT_geo_cor.tif
    input_file = r"F:/HTHT/NJHT/Luren/exercise3/data/H08_20210722_0400_MULT_geo_cor.tif"
    output_path = r"F:/HTHT/NJHT/Luren/exercise3/data/Output/"
    output_file = os.path.join(output_path, "VGT_NDVI_RCUR_202007220400_COOD_H8_AHI_QHS.tif")
    # 重采样
    tiff_resample(input_file, 1, output_path + 'resample_temp.tif')
    resample_file = os.path.join(output_path, 'resample_temp.tif')
    im_data, im_width, im_height, im_bands, im_geotrans, im_proj = GdalBase.read_tiff(resample_file)
    red_band = im_data[2]  # 0.64, reso = 0.5 km
    nir_band = im_data[3]  # 0.86, reso = 1 km
    ndvi = cal_ndvi(nir_band, red_band, im_data[4], im_data[2])
    GdalBase.write_tiff(ndvi, im_width, im_height, 1, im_geotrans, im_proj, output_file)
